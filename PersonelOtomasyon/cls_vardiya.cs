        ...
        public class vardiyalar
        {
            ...
            public void VardiyaEkle(int sicil, DateTime tarih, string yer, string saat)
            {
                using (var baglan = new MySqlConnection(dbConnectionString))
                {
                    ...
                    // Kullanıcıyı sorgula
                    string kullaniciSorgu = $"SELECT * FROM tbl_personel WHERE sicil_no = '{sicil}'";
                    ...
                    using (var kullaniciReader = kullaniciCmd.ExecuteReader())
                    {
                        if (kullaniciReader.Read())
                        {
                            ...
                            string vardiyaEkleSorgu = @"INSERT INTO tbl_vardiya (ad, soyad, sicil_no, tarih_gun, tarih_saat, bolge) VALUES (@ad, @soyad, @sicil_no, @tarih_gun, @tarih_saat, @bolge)";
            ...
            public bool IzinKontrol(int sicilNo, DateTime tarih)
            {
                using (var baglan = new MySqlConnection(dbConnectionString))
                {
                    ...
                    string sorgu = $"SELECT COUNT(*) FROM tbl_izin WHERE sicil_no = '{sicilNo}' AND tarih = '{tarih.ToString("yyyy-MM-dd")}'";
            ...
            public bool IsKontrol(int sicilNo, DateTime tarih)
            {
                using (var baglan = new MySqlConnection(dbConnectionString))
                {
                    ...
                    string sorgu = $"SELECT COUNT(*) FROM tbl_vardiya WHERE sicil_no = '{sicilNo}' AND tarih_gun = '{tarih.ToString("yyyy-MM-dd")}'";
                    var cmd = new MySqlCommand(sorgu, baglan);
            ...
            public bool TatilKontrol(int sicilNo, DateTime tarih)
            {
                using (var baglan = new MySqlConnection(dbConnectionString))
                {
                    ...
                    string kontrolSorgu1 = $"SELECT izin_1 FROM tbl_personel WHERE sicil_no = '{sicilNo}'";
                    string kontrolSorgu2 = $"SELECT izin_2 FROM tbl_personel WHERE sicil_no = '{sicilNo}'";
            ...
            public bool OzelGunKontrol(DateTime tarih)
            {
                using (var baglan = new MySqlConnection(dbConnectionString))
                {
                    ...
                    string sorgu = $"SELECT COUNT(*) FROM tbl_ozelgun WHERE tarih = '{tarih.ToString("yyyy-MM-dd")}'";
                    var cmd = new MySqlCommand(sorgu, baglan);
                ...
                using (var baglan = new MySqlConnection(dbConnectionString))
                {
                    ...
                    using (var kullaniciReader = kullaniciCmd.ExecuteReader())
                    {
                        if (kullaniciReader.Read())
                        {
                            ...
                            string mesaiEkleSorgu = @"INSERT INTO tbl_mesai (ad, soyad, sicil_no, tarih_saat) VALUES (@ad, @soyad, @sicil_no, @tarih_saat)";
            ...
            public bool OtoEkle()
            {
                if (Fark > 0)
                {
                    ...
                    using (var baglan = new MySqlConnection(dbConnectionString))
                    {
                        ...
                        // Kullanıcıyı sorgula
                        string listeSorgu = $"SELECT sicil_no FROM tbl_personel";
